...
class StarRating extends React.Component {
	...
	renderStars() {
		if (this.props.num === 0) {
			...
			for (var j = 0; j < 5; j++) {
				allGrey.push(<img className="rp-star" src={this.state.greyStar} key={j} />);
			}
			return allGrey;
		} else {
			...
			if (fractionNum === 0.25) {
				stars.push(<img className="rp-star" src={this.state[0.25]} key={mainNum} />);
				for (var k = 0; k < leftoverNum - 1; k++) {
					stars.push(<img className="rp-star" src={this.state.greyStar} key={mainNum + 1 + k} />);
				}
			} else if (fractionNum === 0.5) {
				stars.push(<img className="rp-star" src={this.state[0.5]} key={mainNum} />);
				for (var k = 0; k < leftoverNum - 1; k++) {
					stars.push(<img className="rp-star" src={this.state.greyStar} key={mainNum + 1 + k} />);
				}
			} else if (fractionNum === 0.75) {
				stars.push(<img className="rp-star" src={this.state[0.75]} key={mainNum} />);
				for (var k = 0; k < leftoverNum - 1; k++) {
					stars.push(<img className="rp-star" src={this.state.greyStar} key={mainNum + 1 + k} />);
				}
			} else {
